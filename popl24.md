...
We looked at this, and we were inspired by the explosion of interest in program logics for so-called incorrectness reasoning, popularised, of course by Peter O'Hearn's incorrectness logic.  So, we set out to see what it could look like to do incorrectness reasoning in a type system - in other words,  a kind of type system that can give guarantees also when programs are incorrect.  And, what we are proposing is really a fundamental extension to the notion of type system.  So, I hope you will find this work interesting, even if you don't care about incorrectness reasoning.
